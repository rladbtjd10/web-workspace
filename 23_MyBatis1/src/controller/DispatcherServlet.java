...
public class DispatcherServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(mv!=null) {
				if(mv.isRedirect()) {
					response.sendRedirect(mv.getPath()); // true일때 / 리다이텍트 방식으로 보내겠다
				} else {
					request.getRequestDispatcher(mv.getPath()).forward(request, response); //false or 아무것도 아닐때 / 디스패쳐 forward방식으로 보내겠다
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
